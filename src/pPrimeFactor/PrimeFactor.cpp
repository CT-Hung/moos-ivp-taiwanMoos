...
bool PrimeFactor::OnNewMail(MOOSMSG_LIST &NewMail)
{
  ...
  for(p=NewMail.begin(); p!=NewMail.end(); p++) 
  {
    ...
    if (key == "NUM_VALUE")
    {
      ...
      mprime.setReceivedIndex(m_received_index);
      mprime.setCalculatedIndex(0);
      mprime.setDone(false);
      mprime.m_started = false;
      mprime.setStartTime(MOOSTime()); //could've also initilized these with the constructor, unclear if there's a difference or preferred practice here.
